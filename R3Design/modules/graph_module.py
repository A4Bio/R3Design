...
class Attention(nn.Module):
    def __init__(self, num_hidden, num_V, num_E):
        super(Attention, self).__init__()
        self.num_hidden = num_hidden
        self.num_V = num_V
        
        self.W_V = nn.Sequential(
            nn.Linear(num_E+num_V, num_hidden),
            nn.SiLU(),
            nn.Linear(num_hidden, num_hidden),
        )
        self.Bias = nn.Sequential(
        ...
        self.W_O = nn.Linear(num_hidden, num_V)
        self.gate = nn.Linear(num_hidden, num_V)

    def forward(self, h_V, h_E, center_id, batch_id, dst_idx=None):
        w = self.Bias(torch.cat([h_V[center_id], h_E, h_V[dst_idx]],dim=-1))
        attend_logits = w / np.sqrt(int(self.num_hidden)) 

        V = self.W_V(torch.cat([h_E, h_V[dst_idx]], dim=-1))
        attend = scatter_softmax(attend_logits, index=center_id, dim=0)
        h_V = scatter_sum(attend*V, center_id, dim=0)

        h_V_gate = torch.sigmoid(self.gate(h_V))
        h_V_update = self.W_O(h_V) * h_V_gate
        return h_V_update
    

class RDesignLayer(nn.Module):
    def __init__(self, num_hidden, num_in, dropout=0.1, num_heads=None, scale=30):
        super(RDesignLayer, self).__init__()
        self.num_hidden = num_hidden
        self.num_in = num_in
        self.scale = scale
        self.dropout = nn.Dropout(dropout)
        ...
        self.attention = Attention(2*num_hidden, num_hidden, num_hidden) 
        self.edge_update = EdgeMLP(num_hidden, num_in)

    def forward(self, X, h_V, h_E, edge_idx, batch_id=None):
        src_idx, dst_idx = edge_idx[0], edge_idx[1]        
        ...
        h_V_bar = scatter_mean(h_V, batch_id, dim=0)
        h_V = h_V + h_V * self.context_gate(h_V_bar[batch_id])
