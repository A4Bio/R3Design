...
class featurize_GVP:
    ...
    def featurize(self, batch):
        data_all = []
        for b in batch:
            coords = torch.tensor(np.stack([b[c] for c in ['P', 'O5_', 'C5_', 'C4_', 'C3_', 'O3_']], 1))
            ...
            node_v = torch.cat([orientations, sidechains.unsqueeze(-2)], dim=-2).float()

            edge_s = torch.cat([rbf, pos_embeddings], dim=-1).float()
