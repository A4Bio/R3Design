...
from modules.graph_module import gather_nodes, gather_edges, Normalize
...
class RNAFeatures(nn.Module):
    def __init__(self, edge_features, node_features, node_feat_types=[], edge_feat_types=[], num_rbf=16, top_k=30, augment_eps=0., dropout=0.1, args=None):
        ...
        if self.original == 0:
            ...
            self.node_feat_types = node_feat_types = ['angle']
            self.edge_feat_types = edge_feat_types = ['distance', 'direction']
            feat_dims['edge']['distance'] = 16
...
#         V_angle = node_mask_select(self._dihedrals(X))
#         # distance
#         node_list = ['P-C3_', 'O5_-C3_', 'C5_-C3_', 'C4_-C3_', 'O3_-C3_']
#         V_dist = []
#         for pair in node_list:
#             atom1, atom2 = pair.split('-')
#             V_dist.append(node_mask_select(self._get_rbf(vars()['atom_' + atom1], vars()['atom_' + atom2], None, self.num_rbf).squeeze()))
...
#         # dist
#         if self.original == 0:
#             edge_list = ['C3_-C3_', 'P-C3_', 'O5_-C3_', 'C5_-C3_', 'C4_-C3_', 'O3_-C3_']
#         elif self.original == 1:
#             edge_list = ['C3_-C3_']
#         E_dist = [] 
#         for pair in edge_list:
